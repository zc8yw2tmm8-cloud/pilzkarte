...
assert.deepEqual(werte(auswahl), ['eiche']);
assert.deepEqual(werte(ctx.waldAuswahlWechseln(auswahl, 'kiefer')), ['kiefer']);
...
assert.deepEqual(werte(ctx.gespeicherteBaeume({baeume: ['kiefer', 'eiche', 'kiefer']})), ['kiefer']);
...
const map = {
  ...
  addSource(id, source) {assert.equal(sources.size, 0); assert.ok(!sources.has(id)); sources.set(id, source);},
...
assert.equal(legende.children.length, 1);
...
console.log('Baumarten: Einzelauswahl, Gesamtwald und Migration der Einstellungen bestanden.');
